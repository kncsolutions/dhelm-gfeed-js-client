...
var LOCK_GET_HISTORY_OHLC=false;
 ...
 * @name DhelmGfeed
 * @param {Object} params
 * @param {string} params.ws_url The web socket url(required).
 * @param {string} params.api_key The api key you have got on subscription(required).
 * @param {boolean} params.auto_reconnect If false not autoreconnection attempt will be made(optional).
 * @param {int} params.multiplier Multliper to increase the duration of delay for responses with longer length(optional).
 * @param {int} params.synchronization_delay  The delay required to receive response for a request(optional).
 * @param {int} params.function_release_time The systematic delay between subsequent calls to a function(optional). 
 */
var DhelmGfeed = function(params) {
    ...
    if(params.auto_reconnect!=null)auto_reconnect=params.auto_reconnect;
    if(params.multiplier!=null)MULTIPLIER=params.multiplier;
    if(params.synchronization_delay!=null)TIMEOUT=params.synchronization_delay;
    if(params.function_release_time!=null)FORCED_EXIT_TIMEOUT=params.function_release_time;
    var ws = null;
	 ...
	 * @memberOf DhelmGfeed
	 * @method disconnect
     * @param {boolean} force_disconnect User promted disconnect
	 * @instance
	 */
	this.disconnect = function(force_disconnect) {
	 ...
	 * @memberOf DhelmGfeed
	 * @method getHistoricalOHLC
     * @param {object} param 
     * @param {string} param.exchange The exchange(required).
     * @param {string} param.instrument_identifier The instrument identifier(required).
     * @param {number} param.periodicity "HOUR"."MINUTE"."DAY","WEEK", or "MONTH"(required).
     * @param {number} param.from The from time in unix timestamp(required).
     * @param {number} param.to The to time in unix timestamp(required).
     * @param {number} param.max Numerical value of maximum records that should be returned. 
     * Default = 0 (means all available data).(optional).
	 ...
	 * @instance
	 */
    this.getHistoricalOHLC=function(param,callback){
        if(!Authenticated)return;
        if(!LOCK_GET_HISTORY_OHLC){
            console.log("Trying to fetch historical ohlc data.");
            if (param.exchange==null) {
                throw "Invalid Parameter/exchange cannot be null.";
            }
            if (param.instrument_identifier==null) {
            ...
            var forceed_exit_time=Date.now()+FORCED_EXIT_TIMEOUT;        
            count++;
            LOCK_GET_HISTORY_OHLC=true;
            r(7.2,forceed_exit_time);
            if(Authenticated) {
                hisotical_ohlc=constants.RESULT_NOT_PREPARED;
                setTimeout(function(){
                GetHistoricalOHLC(param);
                ws.onmessage=function(e){
                    if(e!=null){
                        if(e.data.includes('"MessageType":"'+constants.MESSAGE_HISTORY_OHLC_RESULT+'"') ){
                            console.log(e.data);
                            hisotical_ohlc=e.data;
                            count--;
                            LOCK_GET_HISTORY_OHLC=false;
                            if(callback)
                               callback(hisotical_ohlc);
                        }
                     }
                };
            },(count>0)?getSynchronizationDelay(7.2):1);
            }
        }
    };
    /**
	 ...
	 * @memberOf DhelmGfeed
	 * @method setAutoReconnectStatus
     * @param {boolean} isTrue Auto reconnection status will be true if set true, else false. 
     */
    this.setAutoReconnectStatus=function(isTrue){
       if(isTrue!=null){
           if(isTrue==true || isTrue==false)auto_reconnect=isTrue;
       }
    }
    /**
     ...
     * @param {number}value multiplier Multiplier value. 
     */
	 ...
	 * @memberOf DhelmGfeed
	 * @method setSynchronizationDelay
     * @param {number} value delay value. 
     */
    this.setSynchronizationDelay=function(value){
        if(value!=null){
            TIMEOUT=value;
        }
     }
	 ...
	 * @memberOf DhelmGfeed
	 * @method setFunctionReleaseTime
     * @param {number} value function release value. 
     */
    this.setFunctionReleaseTime=function(value){
        if(value!=null){
            FORCED_EXIT_TIMEOUT=value;
        }
     }
    ...
    //
    var current_reconnection_count=0;
    var reconnect_max_tries=10;
    var last_reconnect_interval=0;
    var reconnect_max_delay=3000;
    ...
    //
    function GetHistoricalOHLC(param){
        if (ws.OPEN==1) {
            var strMessage = {MessageType:constants.GET_HISTORY,
                              Exchange:param.exchange,
                              InstrumentIdentifier:param.instrument_identifier,
    ...
    function  getSynchronizationDelay(instance_no){
      if(previous_call==3|| previous_call==7.1 || previous_call==7.2){
    ...
    function r(fnIndex,forceed_exit_time){
       // console.log("*****************CLOCK STARTED*************************");
        setTimeout(function(){
           ...
           else if(fnIndex==7.2 && LOCK_GET_HISTORY_OHLC)LOCK_GET_HISTORY_OHLC=false;
