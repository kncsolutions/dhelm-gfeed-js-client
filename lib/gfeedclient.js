 ...
 * @param {int} param.multiplier Multliper to increase the duration of delay for responses with longer length(optional).
 * @param {int} param.synchronization_delay  The delay required to receive response for a request(optional).
 * @param {int} param.function_release_time The systematic delay between subsequent calls to a function(optional). 
 */
var DhelmGfeed = function(params) {
    ...
    if(param.auto_reconnect!=null)auto_reconnect=param.auto_reconnect;
    if(param.multiplier!=null)MULTIPLIER=param.multiplier;
    if(param.synchronization_delay!=null)TIMEOUT=param.timeout;
    if(param.function_release_time!=null)FORCED_EXIT_TIMEOUT=param.function_release_time;
    var ws = null;
     ...
     * 
     */
    this.onResponse=function(param,callback){
       switch (param){
          case 1:
